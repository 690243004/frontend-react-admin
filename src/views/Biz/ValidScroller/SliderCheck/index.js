...
class sliderCheck {
  ...
  initCanvas() {
    ...
    this.slider.css({ left: INITIAL_X + 'px' })
    this.block
      .attr({ src: this.data.slice })
      .css({ left: INITIAL_X + 'px', top: this.data.posY })
  }
      ...
      $.ajax({
        ...
        success: data => {
          if (data.success) {
            ...
            this.el.shake(2, 10, 400) // 窗口抖动
            this.text.fadeIn('normal') // 重现提示文字
            this.slider.animate({ left: INITIAL_X + 'px' }, ANIMATE_SPEED) // 重置滚动条
            this.block.animate({ left: INITIAL_X + 'px' }, ANIMATE_SPEED) // 重置滑块
